...
-- Función 6 - Calcular índice de estacionalidad de precios (usa arreglos y CASE avanzado)
...
DECLARE
    v_tarifas_quarters NUMERIC[4]; -- Array para almacenar tarifas por trimestre
    v_vuelos_quarters INTEGER[4];  -- Array para volumen de vuelos
...
BEGIN
    ...
    FOR v_quarter IN 1..4 LOOP
        ...
        SELECT 
            AVG(ms.fare_avg),
            COUNT(DISTINCT f.flight_id)
        INTO v_tarifas_quarters[v_quarter], v_vuelos_quarters[v_quarter]
        FROM routes r
        JOIN flights f ON r.route_id = f.route_id
        JOIN market_share ms ON f.flight_id = ms.flight_id
        JOIN airports a1 ON r.origin_airport_id = a1.airport_id
        ...
        IF v_tarifa_anual > 0 AND v_tarifas_quarters[v_quarter] IS NOT NULL THEN
            v_indice := v_tarifas_quarters[v_quarter] / v_tarifa_anual;
            v_variacion := ((v_tarifas_quarters[v_quarter] - v_tarifa_anual) / v_tarifa_anual) * 100;
        ELSE
            v_indice := 0;
            v_variacion := 0;
        END IF;
        ...
        SELECT 
            v_quarter,
            ROUND(COALESCE(v_tarifas_quarters[v_quarter], 0), 2),
            ROUND(COALESCE(v_tarifa_anual, 0), 2),
            ROUND(v_indice, 3),
            v_interpretacion,
            COALESCE(v_vuelos_quarters[v_quarter], 0),
            ROUND(v_variacion, 2);
    END LOOP;
...
-- Ejemplo 7: USAR ARRAYS Y CASE AVANZADO - Calcular índice de estacionalidad
...
SELECT DISTINCT city_name FROM cities WHERE city_name IS NOT NULL ORDER BY city_name LIMIT 10;

-- Ver códigos de aerolíneas disponibles
SELECT DISTINCT carrier_code, carrier_type FROM carriers ORDER BY carrier_code LIMIT 10;
